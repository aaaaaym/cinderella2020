...
document.getElementById("submit2").addEventListener("click", function () {
  number++;
  var input1 = document.querySelector("#input13");
  var result = input1.value;
  var checkbox = document.createElement("input13");
  checkbox.className = "custom-control-input2";
  ...
  if (result == "") {
    ...
    document.getElementById("todolist_unit2").appendChild(div);

    //区切り線を引く。
    var line = document.createElement("hr");
    var listline = "listline" + number;
    line.setAttribute("id", listline);
    document.getElementById("todolist_unit2").appendChild(line);

    //フォーム入力し、追加ボタンを押したらフォームをクリアする。
    var reset = document.getElementById("input13");
    reset.value = "";
